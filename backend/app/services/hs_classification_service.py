...
class HSClassificationService:
    ...
    def validate_template(self, df: pd.DataFrame, template_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 1. 헤더 검증 (첫 번째 행)
        required_headers = ["수정여부", "Style No", "이름", "소재타입", "카테고리", "성별", "상세 성분", "HS Code", "Note"]
        ...
        if len(df) == 0:
            ...
            for idx, row in df.iterrows():
                ...
                # StyleNo 필수
                if pd.isna(row.get('Style No')) or str(row.get('Style No')).strip() == '':
                    row_errors.append("StyleNo가 누락되었습니다.")
                
                # 직조방식 필수 및 값 검증
                weaving_type = str(row.get('소재타입', '')).strip().lower()
    ...
    def find_standard_category(self, category_text: str) -> Optional[str]:
        ...
        # 표준 카테고리들과 매칭
        categories = self.db.query(StandardCategory).all()
    ...
    def parse_fabric_composition(self, composition_text: str) -> Dict[str, float]:
        ...
        composition_text = re.split(r'\b(RIB|LINING|ATTACHED)\b', composition_text, flags=re.IGNORECASE)[0]
        
        # SHELL, MAIN 제거
        composition_text = re.sub(r'\b(SHELL|MAIN)\s*\d*%?\s*', '', composition_text, flags=re.IGNORECASE)
        ...
        for comp in components:
            comp_name = comp.component_name_en.upper()
            # 성분명 앞뒤의 숫자를 찾기
            pattern = rf'(\d+(?:\.\d+)?)\s*%?\s*{re.escape(comp_name)}|{re.escape(comp_name)}\s*(\d+(?:\.\d+)?)\s*%?'
            matches = re.findall(pattern, composition_text, re.IGNORECASE)
            
            if matches:
                for match in matches:
                    percentage = match[0] or match[1]
                    if percentage:
                        fabric_components[comp_name] = float(percentage)
                        break
    ...
    def calculate_major_minor_categories(self, fabric_composition: Dict[str, float]) -> Tuple[Optional[str], Optional[str]]:
        ...
        for fabric_name, percentage in fabric_composition.items():
            ...
            if component:
                major_cat = component.major_category_name.lower()
                minor_cat = component.minor_category_name.lower()
    ...
    def classify_products(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for idx, row in df.iterrows():
            result = {
                "row_number": idx + 2,  # 엑셀 행 번호 (헤더 제외)
                "style_no": str(row.get('Style No', '')),
                "product_name": str(row.get('이름', '')),
                "weaving_type": str(row.get('소재타입', '')).strip(),
